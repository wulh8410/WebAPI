...
ps：12306 网页版的 api 不是很稳定，定期会更改，更改套路是上面 url 中的 `leftTicket/query` 后面可能会增加一个大写的字母。例如： `https://kyfw.12306.cn/otn/leftTicket/queryA`，此时返回的 json 中有一个 `c_url` 字段，将其拼接在 `https://kyfw.12306.cn/otn/` 后就是最新的 api 接口。可见 [issue#1](https://github.com/jokermonn/-Api/issues/1)
